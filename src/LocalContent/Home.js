

const content = [
  {
    image: "/ImageFolder/Home/leftImage1.png",
    description:
      "Nadi astrology is specifically designed to guide individuals in pooja and pariharam."
  },
  {
    image: "/ImageFolder/Home/leftImage2.jpeg",
    description:
      "Yes, Nadi astrology predicts past, present, and future using ancient palm-leaf manuscripts."
  },
  {
    image: "/ImageFolder/Home/leftImage3.png",
    description:
      "According to belief, Nadi Astrology predicts your future based on karmic records."
  },
  {
    image: "/ImageFolder/Home/leftImage4.jpeg",
    description:
      "Nadi helps you understand karma and makes your future favorable."
  }
];


const HomeAboutdata = [
        {
            title: "Nadi Astrology",
            image: "/ImageFolder/Home/aboutRight1.png",
            link:"/about_us"

        },
        {
            title: "Our Heritage",
            image: "/ImageFolder/Home/aboutRight2.jpeg",
            link:""
        },
        {
            title: "History of Nadi",
            image: "/ImageFolder/Home/aboutRight3.png",
            link:"/about_us"
        },
        {
            title: "Nadi Procedure",
            image: "/ImageFolder/Home/aboutRight4.png",
            link:""
        },
        {
            title: "Chapters List",
            image: "/ImageFolder/Home/aboutRight5.png",
            link:"/Chapters"
        },
        {
            title: "Online Predictions",
            image: "/ImageFolder/Home/aboutRight6.png",
            link:"/appointment_form"
        }
    ];



    const TestimonyData = [
        {
            content:"சிவநாடி ஜோதிடம் சென்று வந்தேன். என்னுடைய வாழ்க்கையில் நடந்த இறந்தகாலம் நிகழ்காலம் அனைத்தையும் மிக துல்லியமாக கூறுகின்றனர். எனக்கு  ஆச்சரியமாக இருந்தது. எனக்கு மனத்தெளிவு கிடைத்தது அவர்கள் நீண்ட காலம் வாழ இறைவனை மனதார பிரார்த்தனை செய்து கொள்கிறேன். வணக்கமும் வாழ்த்துக்களும் 🙏 I went to Shivnadi Astrology. They tell very accurately everything that has happened in my life past and present. I was surprised. I got clarity and pray to God for long life for them. Greetings and greetings",
            author:"Parthipan"
        },
        {
            content:"Unbelievable predictions.. Sami ji is such a wonderful person.. He is handling people's very humble, respect, polite..and compare to others, cost wise very minimal.. thank you sami",
            author:"Raghu"
        },
        {
            content:"Very good experience, its remarkable one. Guruj fully explained my past, present, future. The predictions are 100%matched with my life.",
            author:"Mahesh Sathiya"
        },
        {
            content:"Very best Nadi online service excellent. present and past great experience too all prediction my life very important and useful jothidam long live my children way of life nadijothidam so iam very happy. ",
            author:"Raaja Mohan"
        },
        {
            content:"I have been visiting their office frequently for the past six years and their service is incredible.everything is accurate and all the remedies they are suggesting are working properly.I suggest anyone who wants to see Nadi astrology consultation to completely trust them and do all the remedies written by the great saints who knew past ,present and future accurately each and everything in detail.my experience is awesome and I’m awestruck by the way everything is explained .may god bless them and wish them all success for their future endeavours.",
            author:"Ganapathy Srinivasan"
        },
        {
            content:"I'm visiting that place. The most useful in our future. All the words they said will be need to our future. Definitely visit this place they are peaceful and kindness to conduct jothidam. i attended the program. Its very usefull to enlighten my knowledge! Thanks for the opportunity. ",
            author:"Jeeva"
        }
    ]



    const DetailComponentData = [
        {   image:"/ImageFolder/Home/AuthenticReading.png",
            title: "AUTHENTIC READING",
            description: "Get your Nadi reading from the original Nadi family"
        },
        {   image:"/ImageFolder/Home/Languages.png",
            title: "AVAILABLE IN MAJOR LANGUAGES",
            description: "தமிழ், English, Hindi, Telugu, Kanada"
        },
        {   image:"/ImageFolder/Home/TakeToUs.png",
            title: "TALK TO US",
            description: "+91 9566381222"
        }
    ];



export  {content,HomeAboutdata,TestimonyData,DetailComponentData};